...
public class PridajSlovicka extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		nazvyKategorii.addAll(db.kategorie(dieta));
		...
		help = (EditText) findViewById(R.id.HelpEdit);
		...
		listview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int position,
				...
				help.setText(slova.get(position).getHelp());
				upravujem = true;
				upravovane = position;
			}
	...
	public void save(View view){
		if (upravujem){
			if (anglicky.getText().toString().isEmpty() || slovensky.getText().toString().isEmpty()){
				...
				slova.get(upravovane).setHelp(help.getText().toString());
				item.put( "sk", slova.get(upravovane).getSk());
				item.put( "en", slova.get(upravovane).getEn() + " (" + slova.get(upravovane).getHelp() + ")");
		    ...
		    item.put( "en", slovo.getEn()+ " (" + help.getText().toString() + ")");
		    slovo.setHelp(help.getText().toString());
		    slovo.setName(dieta);
		    slova.add(slovo);
		    list.add(item);
	...
	public void vybrat(View view){
		...
		if (kat.equals("Nová kategória")){
			...
			help.setVisibility(1);
			nova = false;
			slova = db.slovaZoSuboru(kat, dieta);
			
			HashMap<String,String> item;
			for(int i=0;i<slova.size();i++){
			    item = new HashMap<String,String>();
			    item.put( "sk", slova.get(i).getSk());
			    item.put( "en", slova.get(i).getEn() + " (" + slova.get(i).getHelp() + ")");
			    list.add( item );
			}
			sa.notifyDataSetChanged();
